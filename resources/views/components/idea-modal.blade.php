            ...
            <div>
                <fieldset class="space-y-3">
                    ...
                    <div class="flex gap-x-2 items-center">
                        <input type="text" placeholder="Enter a Step for the idea" id="new-step" autocomplete="off"
                            class="input flex-1" spellcheck="false" x-model="newStep" data-test="new-step"
                            aria-label="New step"
                        ...
                        <button type="button" :disabled="newStep.trim().length === 0" class="btn btn-outlined h-10"
                            ...
                            <span class="sr-only" data-test="add-step">Add step</span>
                        </button>
                    </div>
                    ...
                    <div role="list" aria-live="polite" aria-label="Added steps">
                        <template x-for="(step, index) in steps" :key="index">
                            <div class="flex gap-x-2 items-center border border-border rounded-lg p-3 my-2"
                                ...
                                <button type="button" @click="steps.splice(index, 1)" data-test="remove-step-button"
                                    :aria-label="'Remove step ' + step">
                                    <x-icons.close aria-hidden="true" />
                                    <span class="sr-only" data-test="remove-step">Remove step</span>
                                </button>
                            </div>
                        </template>
                    </div>
            ...
            <div>
                <fieldset class="space-y-3">
                    ...
                    <div class="flex gap-x-2 items-center">
                        ...
                        <button type="button" :disabled="newLink.trim().length === 0" class="btn btn-outlined h-10"
                            @click="links.push(newLink.trim()); newLink = ''" data-test="add-link-button"
                            aria-label="Add link to list">
                            <x-icons.close class="rotate-45" aria-hidden="true" />
                            <span class="sr-only" data-test="add-link">Add link</span>
                        </button>
                    </div>
                    ...
                    <div role="list" aria-live="polite" aria-label="Added links">
                        <template x-for="(link, index) in links" :key="index">
                            <div class="flex gap-x-2 items-center border border-border rounded-lg p-3 my-2"
                                ...
                                <button type="button" @click="links.splice(index, 1)" data-test="remove-link-button"
